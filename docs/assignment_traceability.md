...
| **Conversational** (clarifying Qs, follow-ups) | `await_followup`, CLI `deepen`/`challenge`/`retry` | ambiguous identity asks a question | ◐ (follow-ups Phase 4) |
...
| Markdown brief: how they position products | product family + positioning | product/pricing section | ◐ (product depth Phase 3) |
| Markdown brief: what changed recently | temporal engine + change events | both-period change events | ✔ |
...
| Agent output for ≥1 competitor | reference Deel-vs-Rippling run | saved brief + JSON + trace | ◐ (live run) |
| Loom: architecture, tools/models, evals, live demo | `ARCHITECTURE.md`, ADRs, `docs/demo_runbook.md`, eval report | runbook + trace + evals | ◐ (Phase 6/7) |
| Graceful tool failures | `tools/base.py` boundary | exception → typed result; fallback in trace | ✔ |
| Eval system | `evals/` (classification, grounding) + 50-artifact set | held-out scoring; adjudication guide | ○ (Phase 6, hard stop) |

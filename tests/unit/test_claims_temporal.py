...
def _ev(eid: str, excerpt: str = "all-in-one workforce platform") -> EvidenceItem:
    return EvidenceItem(
        evidence_id=eid, artifact_id="art1", company_id="c1",
        exact_excerpt=excerpt, normalized_statement="stmt", evidence_type="positioning",
        source_quality="high", extraction_confidence="high",
    )
...
async def test_builder_strips_unknown_evidence_and_performance_claims():
    ...
    output = ClaimBuilderOutput(
        claims=[
            CandidateClaim(statement="Deel leads with an all-in-one platform message",
                           supporting_evidence_ids=["EV1", "GHOST"], proposed_status="observed",
                           suggested_confidence="high"),
            CandidateClaim(statement="Their ads have a 3x ROAS",
                           supporting_evidence_ids=["EV1"], proposed_status="observed"),
            CandidateClaim(statement="No support here", supporting_evidence_ids=["GHOST"]),
        ]
    )
    claims = await build_claims(
        [_ev("EV1")], FakeGateway(output), PROMPTS, company_name="Deel", company_id="c1",
...
async def test_judge_applies_acceptance_policy(verdict, expected_status):
    ...
    claim = StrategicClaim(
        claim_id="CLM1", company_id="c1", statement="Deel is all-in-one",
        status="hypothesis", evidence_ids=["EV1"], claim_confidence="high",
        research_coverage="low", source_quality="high", confidence_reason="x",
    )
    v = ClaimJudgeVerdict(verdict=verdict, narrowed_claim="Deel markets multiple products", reason="r")
